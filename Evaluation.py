...
for pro in pro_labels:
    ...
    for lig in lig_labels:
        ...
        y_p = model.predict(X)
        if y_p[0][1] > 0.5:
            output['predicted_lig_label_and_score'].append((lig, y_p[0][1]))
        output['predicted_lig_label_and_score'] =sorted(output['predicted_lig_label_and_score'],key=getKey,reverse=True)[:10]
    output['found'] = True if pro in [a[0] for a in output['predicted_lig_label_and_score']] else False
